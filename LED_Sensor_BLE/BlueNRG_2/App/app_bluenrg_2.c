...
/* USER CODE BEGIN PV */
extern IKS02A1_MOTION_SENSOR_Axes_t acceleration;
extern IKS02A1_MOTION_SENSOR_Axes_t angular_velocity;
extern IKS02A1_MOTION_SENSOR_Axes_t magnetic_field;
/* USER CODE END PV */
  ...
  if (user_button_pressed)
  {
    ...
    if (paired)
    {
      ...
      if ((send_mot) || (send_quat)) {
        ...
        if (send_quat) {
            q_axes.AXIS_X =  9*acceleration.x;// + angular_velocity.x;
            q_axes.AXIS_Y = 9*acceleration.y;
            q_axes.AXIS_Z = 9*acceleration.z;
            Quat_Update(&q_axes);
        }
        counter ++;
        if (counter == 40) {
